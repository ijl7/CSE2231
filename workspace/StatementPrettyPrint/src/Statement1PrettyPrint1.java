        ...
        switch (this.kind()) {
            case BLOCK: {
                ...
                while (i < this.lengthOfBlock()) {
                    Statement lower = this.removeFromBlock(i);
                    lower.prettyPrint(out, offset);
                    this.addToBlock(i, lower);
                    i++;
                }
                break;
            }
            case IF: {
                ...
                Statement lower = this.newInstance();
                StatementKernel.Condition cond = this.disassembleIf(lower);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(cond) + " THEN");
                lower.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIf(cond, lower);
                break;
            }
            case IF_ELSE: {

                Statement first = this.newInstance();
                Statement second = this.newInstance();
                StatementKernel.Condition cond = this.disassembleIfElse(first,
                        second);
                printSpaces(out, offset);
                out.println("IF " + toStringCondition(cond) + " THEN");
                first.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("ELSE");
                second.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END IF");
                this.assembleIfElse(cond, first, second);
                break;
            }
            case WHILE: {

                Statement lower = this.newInstance();
                StatementKernel.Condition cond = this.disassembleWhile(lower);
                printSpaces(out, offset);
                out.println("WHILE " + toStringCondition(cond) + " DO");
                lower.prettyPrint(out, offset + Program.INDENT_SIZE);
                printSpaces(out, offset);
                out.println("END WHILE");
                this.assembleWhile(cond, lower);
